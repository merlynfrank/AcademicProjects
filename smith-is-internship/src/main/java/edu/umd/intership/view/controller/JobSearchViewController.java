...
public class JobSearchViewController {
	...
	private String searchJob(@ModelAttribute(name = "jobSearchForm") JobSearchForm jobSearchForm, Model model) {
		...
		String jobTitle ="*";
		String location ="*";
		if(jobSearchForm.getJobType()!=null && !jobSearchForm.getJobType().equalsIgnoreCase("Select"))
			jobType = jobSearchForm.getJobType();
		if(jobSearchForm.getJobTitle()!=null && !jobSearchForm.getJobTitle().equalsIgnoreCase("Select"))
			jobTitle = jobSearchForm.getJobTitle();
		if(jobSearchForm.getJoblocation()!=null && !jobSearchForm.getJoblocation().equalsIgnoreCase("Select"))
